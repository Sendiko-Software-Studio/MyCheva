...
class CommentRepositoryImpl @Inject constructor(
    ...
    override suspend fun loadData(token: String, forumId: String): Result<GetForumResponse> {
        return suspendCoroutine { continuation ->
            apiServices.getForum(token, forumId)
                .enqueue(
    ...
    override suspend fun postReply(token: String, request: PostReplyRequest): Result<PostReplyResponse> {
        return suspendCoroutine { continuation ->
            apiServices.postReply(token, request)
                .enqueue(
